...
gpio_num_t ButtonGpio(ZectrixButton button) {
    for (const ButtonDefinition& definition : kButtons) {
        if (definition.button == button) {
            return definition.gpio;
        }
    }
    return GPIO_NUM_NC;
}
...
esp_err_t ZectrixHoldBatteryPowerEarly() {
    gpio_deep_sleep_hold_dis();
    esp_err_t err = gpio_hold_dis(ZECTRIX_VBAT_LATCH);
    if (err != ESP_OK) {
        return err;
    }
    // Preload the output latch before enabling the pad as an output.
    err = gpio_set_level(ZECTRIX_VBAT_LATCH, 1);
    if (err != ESP_OK) {
        return err;
    }
    gpio_config_t latch = {};
    latch.pin_bit_mask = 1ULL << ZECTRIX_VBAT_LATCH;
    latch.mode = GPIO_MODE_OUTPUT;
    latch.pull_up_en = GPIO_PULLUP_DISABLE;
    latch.pull_down_en = GPIO_PULLDOWN_DISABLE;
    latch.intr_type = GPIO_INTR_DISABLE;
    err = gpio_config(&latch);
    if (err != ESP_OK) {
        return err;
    }
    err = gpio_set_level(ZECTRIX_VBAT_LATCH, 1);
    ...
    return gpio_hold_en(ZECTRIX_VBAT_LATCH);
}
...
esp_err_t ZectrixBoard::InitPowerAndGpio() {
    esp_err_t err = ZectrixHoldBatteryPowerEarly();
    if (err != ESP_OK) {
        return err;
    }

    gpio_config_t outputs = {};
    outputs.pin_bit_mask = (1ULL << ZECTRIX_AUDIO_POWER) |
                           (1ULL << ZECTRIX_POWER_LED) |
                           (1ULL << ZECTRIX_NFC_POWER);
    ...
    gpio_set_level(ZECTRIX_NFC_POWER, 0);
...
esp_err_t ZectrixBoard::Init() {
    ...
    ESP_LOGI(kTag, "board initialized rtc=%d", rtc_ != nullptr);
    return ESP_OK;
}
...
bool ZectrixBoard::IsButtonPressed(ZectrixButton button) const {
    const gpio_num_t gpio = ButtonGpio(button);
...
void ZectrixBoard::PrepareDeepSleepHold() {
