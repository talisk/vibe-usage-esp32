...
typedef enum {
    ...
    BSP_BTN_DOUBLE,      // 双击
    BSP_BTN_LONG,        // 长按
    BSP_BTN_RELEASE,     // 松开；语音结束只设原子标志，不能阻塞
} bsp_btn_ev_t;
...
// Uses the existing shared ADC button driver; does not allocate another ADC
// owner. Intended for boot-time hold detection.
bool bsp_button_is_pressed(bsp_btn_t btn);
