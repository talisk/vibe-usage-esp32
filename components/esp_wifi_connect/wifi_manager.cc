...
bool WifiManager::Initialize(const WifiManagerConfig& config) {
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "NVS init failed without destructive recovery: %s",
                 esp_err_to_name(ret));
        return false;
    }
...
void WifiManager::SetLanguage(const std::string& language) {
    std::lock_guard<std::mutex> lock(mutex_);
    config_.language = language; // Applied when the next portal session starts.
}
...
void WifiManager::StartConfigAp() {
    ...
    config_ap_->SetProjectInfo(config_.project_info);
